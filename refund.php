...
foreach ($refundData as $key => $value) {
	if(isset($value[1])) {
		...
		echo "\n".'orderid:' . $orderId ."\n";
		$verslag .= $timenow. ',' .$orderId;
		if($returnData->success) {
			echo "OK\n";
			$verslag .= ",OK\n";
		} else {
			echo "Error: ".$returnData->error_code . ' ' . $returnData->error_info . "\n";
			$verslag .= ",Error: ".$returnData->error_code . ' ' . $returnData->error_info . "\n";
		};
